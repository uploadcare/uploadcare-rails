# frozen_string_literal: true

require "uploadcare/rails/api/rest/base"

module Uploadcare
  module Rails
    module Api
      module Rest
        # A class that contains File related methods for Uploadcare REST API
        class FileApi < Base
          class << self
            # Returns a pagination json of files stored in project
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/filesList
            #
            # valid options:
            # removed: [true|false]
            # stored: [true|false]
            # limit: (1..1000)
            # ordering: ["datetime_uploaded"|"-datetime_uploaded"]
            # from: A starting point for filtering files. The value depends on your ordering parameter value.
            def get_files(options = {}, config: Uploadcare.configuration)
              Uploadcare::File.list(options: options, config: config)
            end

            # Acquire file info
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/fileInfo
            def get_file(uuid, config: Uploadcare.configuration)
              Uploadcare::File.info(uuid: uuid, config: config)
            end

            # 'local_copy' method is used to copy original files or their modified versions to a default storage.
            # Source files MAY either be stored or just uploaded and MUST NOT be deleted.
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#tag/File/operation/createLocalCopy
            def local_copy_file(source, options = {}, config: Uploadcare.configuration)
              Uploadcare::File.local_copy(source: source, options: options, config: config)
            end

            # 'remote_copy' method is used to copy original files or their modified versions to a custom storage.
            # Source files MAY either be stored or just uploaded and MUST NOT be deleted.
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#tag/File/operation/createRemoteCopy
            def remote_copy_file(source, target, options = {}, config: Uploadcare.configuration)
              Uploadcare::File.remote_copy(source: source, target: target, options: options, config: config)
            end

            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/deleteFile
            def delete_file(uuid, config: Uploadcare.configuration)
              file = Uploadcare::File.new({ uuid: uuid }, config)
              file.delete
              file
            end

            # Store a single file, preventing it from being deleted in 2 weeks
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/storeFile
            def store_file(uuid, config: Uploadcare.configuration)
              file = Uploadcare::File.new({ uuid: uuid }, config)
              file.store
              file
            end

            # Make a set of files "stored". This will prevent them from being deleted automatically
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/filesStoring
            # uuids: Array
            def store_files(uuids, config: Uploadcare.configuration)
              Uploadcare::File.batch_store(uuids: uuids, config: config)
            end

            # Delete several files by list of uids
            # @see https://uploadcare.com/api-refs/rest-api/v0.7.0/#operation/filesDelete
            # uuids: Array
            def delete_files(uuids, config: Uploadcare.configuration)
              Uploadcare::File.batch_delete(uuids: uuids, config: config)
            end
          end
        end
      end
    end
  end
end

# Backward-compatible alias for file REST API client.
Uploadcare::FileApi = Uploadcare::Rails::Api::Rest::FileApi
